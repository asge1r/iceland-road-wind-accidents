...
| O/E denominator | Expected accidents are calculated within station, calendar year and season from local 10-minute wind frequency. | `src/weather/frequency.py`; `src/analysis/build_oe.py`; `docs/pipeline.md` |
| Compact data for review | Routine inspection and the core analysis use eight small files in `data/analysis/`. Raw deliveries and large reproducible caches are separate. | `data/analysis/README.md`; `data/analysis/manifest.csv`; `docs/data.md` |
