...
Matched-time and allocated daily-counter models provide secondary robustness checks.
...
The largest excluded category is uninterrupted all-zero runs, which
...
\citep{vegagerdinTraffic}. In principle, summer and winter averages and a
residual spring/autumn average could be spread over their respective periods
to approximate exposure across the full study period. This is not the main
approach used here; these averages are not used in the main analysis.
...
response and the counter-linked exposure calculation, but do
...
2019--2024; the broader weather sample covers 2007--2025.
The first four rows follow the accident selection sequentially. The last three
rows describe separate selections for counter-linked accidents, weather
observations and counter-days.
...
separate severity groups used in the O/E figures. These are descriptive counts:
...
Serious or fatal injury accidents are more numerous in summer than winter,
while the minor-injury counts are similar. The longer summer and winter
periods and differences in travel must be considered when reading these counts.
...
serious or fatal injury (codes 1--2). Spring and autumn are combined only in this descriptive figure. Hour,
season and daylight use 6,414 accidents; temperature uses 6,259 qualifying
matches. Temperature brackets denote displayed interval endpoints; bins include the lower endpoint and exclude the upper. These are counts without traffic adjustment.}
...
the weather-only analysis. The correction reweights this comparison using the traffic response
estimated from the 2019--2024 counters. The traffic-based rate analysis uses the counter-linked
...
\subsection{Weather-only Observed / Expected Analysis}
\label{sec:weather-frequency}

The observed-to-expected ratio (O/E) compares the number of accidents observed
in each weather interval with the number expected from local weather frequency.
...
averaging the two group ratios. The weather-only analysis is reported as a descriptive standardised
...
12\(^{\circ}\)C. In all temperature figures, square brackets show the
interval endpoints as a display convention: each finite bin includes its lower
endpoint and excludes its upper endpoint.
The O/E upper interval is combined at 12\(^{\circ}\)C to keep the seasonal
panels readable. The matched-time models retain separate
...
spatial and temporal resolution, so the weather-only analysis does not account for changes in travel
...
Traffic enters the main analysis in two ways. The approximate correction estimates the relative daily
traffic response across weather intervals from the 2019--2024 counters and uses
that response to reweight weather-only denominator. The traffic-based rate analysis constructs estimated
...
An allocated daily-counter model is retained as a supporting check under
an alternative exposure construction.
...
Using the same station--season group \(g\) and weather interval \(j\) as in the weather-only analysis,
...
five-minute time rule applies as in the weather-only analysis and can select the adjacent observation
...
keeps the finer bins. This is the traffic-rate analysis's exposure-allocation sensitivity, distinct
...
(Figure~\ref{fig:annual-coverage}).

Figure~\ref{fig:annual-counts} shows a decline spanning the financial-crisis
period and a lower count in 2020 during the COVID-19 period, followed by a
partial recovery. The dip in 2014 is also visible. The official accident report
documents the reporting system and earlier changes but does not establish a
2014 reporting change that explains this dip \citep{itaReport2014}.
These time patterns alone do not identify their causes or separate changes
in travel from changes in accident risk.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{annual_accident_counts.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{weather_coverage.png}
\caption{Annual weather-match coverage among rural injury accidents, 2007--2025.
...
The traffic-based analysis uses a smaller, counter-linked population.
Table~\ref{tab:monthly-vkt-selection} follows selection to the final 694
accidents. All 83 exclusions at the final step lack a counter-day record on
...
This motivates an approximate correction under the assumption that the
traffic response observed at the selected counters is
...
intervals, so the approximate correction applies the estimated counter response to the primary
...
for mean wind and from 3.34 to 4.20 for gust. Outlined bars show the weather-only analysis and filled
teal bars show the approximate correction for the same all-injury population. Thus, accounting for lower
...
separately for minor injury and serious or fatal injury, using the same two groups as the weather-only comparison. This shows whether the corrected upper-wind pattern is shared across
...
Both constructions are defined in the Methods.
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth,height=0.78\textheight,keepaspectratio]{monthly_f_traffic_rate_panels.png}
\caption[Traffic-based mean wind rates by season.]{Rural injury accident rates by mean wind, 2019--2024.
Light blue and red show disjoint minor and serious or fatal injury rates sharing
estimated vehicle-km from daily traffic, rural length and pooled monthly weather.
All year retains fine bins; seasonal tails combine intervals from 15 m/s.
Positive labels give accident counts. Sparse cells and unobserved hourly traffic
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth,height=0.78\textheight,keepaspectratio]{monthly_fg_traffic_rate_panels.png}
\caption[Traffic-based gust rates by season.]{Rural injury accident rates by gust, 2019--2024.
Light blue and red show disjoint minor and serious or fatal injury rates sharing
estimated vehicle-km from daily traffic, rural length and pooled monthly weather.
All year retains fine bins; seasonal tails combine intervals from 20 m/s.
Positive labels give accident counts. Sparse cells and unobserved hourly traffic
limit interpretation.}
...
for the three main analyses. Read each estimate against its stated denominator: the table is
...
\begin{figure}[p]
\centering
\includegraphics[width=\textwidth,height=0.78\textheight,keepaspectratio]{weather_rate_annual.png}
...
\begin{figure}[p]
\centering
\includegraphics[width=\textwidth,height=0.78\textheight,keepaspectratio]{f_traffic_rate_panels.png}
...
\begin{figure}[p]
\centering
\includegraphics[width=\textwidth,height=0.78\textheight,keepaspectratio]{fg_traffic_rate_panels.png}
...
\begin{figure}[p]
\centering
\includegraphics[width=\textwidth,height=0.78\textheight,keepaspectratio]{temperature_traffic_rate_panels.png}
...
models. The allocation sensitivity above and period comparisons below examine exposure assumptions and period choice; the following checks test other aspects of the wind association.
...
or replacements for the full-period weather-only figures. Upper-bin counts should be read
...
The line marks O/E = 1. This supporting period comparison contains no traffic adjustment.}
...
the approximate correction's full-period estimates. Its temperature panel concerns
corrected weather-frequency O/E, not the traffic-based rate method.
...
not the warm-end increase seen in the weather-only analysis, reinforcing the cautious interpretation
of temperature.
...
The supporting allocated daily-counter model also shows higher rates in strong
wind. Its sample and exposure construction differ from the main traffic-rate
analysis. Appendix Table~\ref{tab:traffic-sensitivity} checks how recorded zero
counter-days affect the estimated traffic response.
...
intervals. The weather-only analysis establishes over-representation relative to local weather
frequency; the approximate correction asks whether reduced traffic explains it; the traffic-based rate analysis relates accidents
...
The approximate correction addresses an important limitation of weather-frequency O/E: traffic volume
...
intervals used to classify accidents. The approximate correction is therefore best interpreted as a
sensitivity analysis of the primary O/E rather than as a measured
traffic-adjusted accident rate.

The traffic-based rate analysis incorporates day-to-day traffic variation directly in the denominator, but
...
traffic-based rate ratios. This suggests that differences in road and exposure composition
...
The approximate correction tests how the primary result changes under an empirically estimated traffic
response, while the traffic-based rate analysis uses observed daily traffic, road length and all eligible
...
The main limitation is incomplete traffic exposure. The weather-only analysis uses weather time rather
than vehicle-kilometres. The approximate correction introduces traffic only through an approximate
response estimated from selected counters in 2019--2024 and extrapolated to the
broader 2007--2025 population. The counter data are daily totals, so neither the approximate correction
nor the traffic-based rate analysis observes how many vehicles were travelling during the specific
10-minute weather interval in which an accident occurred.

The traffic-based rate analysis is more exposure-based but also more selective. It is limited to rural
...
accidents themselves are restricted to 2019--2024. Few the traffic-based rate analysis accidents occur
...
conditions, visibility or closures on a particular day. More generally, all
