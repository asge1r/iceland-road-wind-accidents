...
def plot_one_variable(data: pd.DataFrame, variable: str, path: Path) -> None:
    ...
    axis.set_xlabel(VARIABLE_XLABELS[variable], fontsize=16)
    axis.set_ylabel("Accident O/E ratio", fontsize=16)
    axis.tick_params(axis="both", labelsize=14)
    axis.grid(axis="y", alpha=0.2)
    top = max(1.5, np.nanmax(values) * 1.18)
    axis.set_ylim(0, top)
    for bar, count in zip(bars, subset["observed_accidents"], strict=True):
        axis.text(bar.get_x() + bar.get_width() / 2, max(bar.get_height() * 0.55, top * 0.06), f"n={int(count)}", ha="center", va="center", fontsize=12, color="white")
...
def plot_primary(data: pd.DataFrame, path: Path) -> None:
    ...
    axis.axhline(1, color="#222222", linestyle="--", linewidth=1)
    axis.set_ylabel("Accident O/E ratio", fontsize=17)
    axis.set_xlabel("Mean wind, f (m/s)", fontsize=17)
    axis.set_xticks(x, interval_labels(subset["coarse_bin"]))
    ...
    axis.set_ylim(0, top)
    for bar, count in zip(bars, subset["observed_accidents"], strict=True):
        axis.text(bar.get_x() + bar.get_width() / 2, max(bar.get_height() * 0.55, top * 0.06), f"n={int(count)}", ha="center", va="center", fontsize=13, color="white")
    figure.savefig(path, dpi=240)
    plt.close(figure)
